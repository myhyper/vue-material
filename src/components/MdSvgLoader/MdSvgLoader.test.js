...
test("should change the current SVG to another", async () => {
  ...
  const svgContent2 =
    '<svg width="24" xmlns="http://www.w3.org/2000/svg"></svg>';
  const prop = "mdSrc";
  let wrapper = null;
...
test("should throws if cors or an unknown error", async () => {
  const svgUrl =
    "https://storage.googleapis.com/material-icons/external-assets/v4/icons/svg/ic_3d_rotation_black_24px.svg";
  let wrapper = null;
  ...
  expect(wrapper.vm.error).toBe(
    `Something bad happened trying to fetch ${svgUrl}.`
  );
});
